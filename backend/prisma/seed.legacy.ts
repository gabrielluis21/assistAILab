...
/**
 * ============================================================
 * ASSISTAILAB — LEGACY MANUAL FRONTEND TEST SEED (NOT THE DEFAULT)
 * ============================================================
 *
 * Objetivos:
 *
 ...
 * IMPORTANTE:
 *
 * Dataset LEGADO para demonstração manual em banco separado, com a API
 * parada. Não é pré-requisito nem fixture da suíte automatizada FE-02B.
 * PART permanece global no banco legado, mas não é publicado pelo Sync.
 * Não executar depois de inicializar Sync FE-02B: as escritas diretas
 * deste seed não participam da barreira transacional das projeções.
 */
...
async function main() {
    // Read-only preflight: reject incompatible targets before the first write.
    await assertManualSeedTarget(process.env, async () => Boolean(
        await prisma.syncChangeLog.findUnique({
            where: { cursor: '__FE02B_PROJECTION_BARRIER__' },
            select: { id: true },
        })
    ));
    console.log('');
    ...
    // Catalog rows are legacy demo evidence only. PART has no Sync authority
    // in either contract version until SEC-PART-TENANCY-01.
    ...
    console.log('');
    console.log(
        'Dataset legado manual. Para FE-02B, preservar a Outbox e executar o bootstrap v2 completo; não apagar o SQLite para simular bootstrap.'
    );
    console.log('');
}
